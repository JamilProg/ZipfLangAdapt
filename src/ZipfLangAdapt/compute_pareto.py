...
def main(input_data: str = os.path.join('.', 'data', 'out_prefixes_ranks.csv'), output_data: str = os.path.join('.', 'data', 'optimum_theorique.csv')):
    ...
    for i, row in df.iterrows():
        ...
        pareto_points, pareto_labels = compute_pareto_front(
            values_k, values_j, prefixes)
        optidict[fulllabel] = (hid, label, pareto_points, pareto_labels)
        print(count)
        count += 1
    df = pd.DataFrame(
        columns=["HID", "LABEL", "PARETO_OPTI", "PARETO_OPTI_COORD"])
    for _, (hid, label, paretop, paretol) in optidict.items():
        # df = df.append({'HID': hid, 'LABEL': label, "PARETO_OPTI": paretol,
        #                "PARETO_OPTI_COORD": paretop}, ignore_index=True)
        df = pd.concat([df, pd.DataFrame.from_records([{'HID': hid, 'LABEL': label, "PARETO_OPTI": paretol, "PARETO_OPTI_COORD": paretop}])], ignore_index=True)
